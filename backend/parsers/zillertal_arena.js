...
export async function parse(options = {}) {
    ...
    const apiUrl = "https://winter.intermaps.com/zillertalarena/data?lang=de";

    const res = await fetchWithHeaders(apiUrl, options);
    if (!res.ok) {
        throw new Error(`Failed to fetch Zillertal Arena API: ${res.status}`);
    }
    ...
    if (!data || !Array.isArray(data.lifts)) {
        throw new Error("Invalid Zillertal Arena API response: 'lifts' array missing");
    }
    ...
    return createResult(details.id, { liftsOpen, liftsTotal, lifts, slopes }, "Intermaps (zillertalarena.com)");
}
